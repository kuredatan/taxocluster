...
#@idSequences is a dictionary of (key=identifier of node,value=((name,rank of node),phylogeny of node))
...
def getMatchingNodes(allMatches,idSequences,filenames):
    matchingNodes = dict.fromkeys((None,None))
    for sample in filenames:
        ...
        matchingNodes.setdefault(sample,matchingNodesInThisSample)
...
def featuresCreate(filenames,fastaFileName):
    ...
    print "[ You may have to wait a few minutes... ]"
    try:
        #@idSequences is a dictionary of (key=identifier,value=((name,rank),phylogeny of node))
